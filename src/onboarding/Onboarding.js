...
const Onboarding = ({ setShowHomePage }) => {
  ...
  const [activeSlideIndex, setActiveSlideIndex] = useState(0); // State to keep track of active slide index
  const sliderRef = useRef(null);

  useEffect(() => {
    fetchSlides(1); // Fetch slides starting from index 1
  }, []);
  ...
  const fetchSlides = async (startIndex) => {
    try {
      const requestData = {
        eventID: "1002",
        addInfo: {
          id: startIndex.toString()  // Dynamically set id based on active slide index (starting from 1)
        }
      };
  ...
  const handleNext = () => {
    if (sliderRef.current) {
      ...
      setActiveSlideIndex(activeSlideIndex + 1); // Update activeSlideIndex when navigating to next slide
    }
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      <AppIntroSlider
        ...
        renderSkipButton={() => renderButton('Skip', handleSkip)}
        renderDoneButton={() => renderButton('Done', handleSkip)}
        onSlideChange={(index) => setActiveSlideIndex(index)} // Update activeSlideIndex on slide change
        onDone={() => setShowHomePage(true)}
      />
    </View>
  );
